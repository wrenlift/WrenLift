...
// Above any 64-bit user-space address (48-bit VA on x86_64/aarch64,
// 52 bits leaves room for larger layouts) yet far below the NaN-box
// tag space, so a Value mistakenly traced as a header pointer lands
// above the cap and gets skipped. wasm32 is 32-bit (usize = u32 = max
// 4 GB), so the cap doesn't fit the type; on that target every
// flat-address pointer is by definition valid, so set the cap to
// usize::MAX (i.e. no upper bound check) and let the lower bound +
// as_object/is_object guards handle false positives.
