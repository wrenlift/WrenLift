...
pub fn x86_64_target_regs() -> TargetRegs {
    // All GP except RSP(4), RBP(5), R10(10=spill scratch), R11(11=call+copy scratch).
    // R11 is also SCRATCH_GP in the x86_64 emitter (used by AndImm/ICmpImm for
    // 64-bit immediates). The boxed arith/cmp fast paths now use explicit LoadImm +
    // register-register And/ICmp instead of AndImm/ICmpImm, avoiding the conflict.
