...
Symptom. Every `[dependencies]` entry has to pin an exact
version literal (`"@hatch:gpu" = "0.2.7"`). There's no
`"^0.2.0"` / `">=0.2.5,<0.3"` / `"*"` syntax. As a result,
patch-level publishes downstream of a depended-on package need
a manual cascade of bumps: gpu 0.2.6 → 0.2.7 forces game to
republish 0.2.9 just to track the gpu pin.

Root cause. `Dependency::Version(String)` is parsed as a
literal version that the resolver feeds straight into
`release_url(name, version)`; both the host CLI's git cache
walker and the wasm dep walker treat the string as the exact
GitHub-release tag fragment.

The registry has no list-versions API today — `release_url`
constructs a fixed URL per `(name, version)` pair, and there
is no manifest the resolver can ask "which versions of
`@hatch:foo` exist?"

Fix sketch.
1. Index file at the registry root listing every published
   `(name, version)` pair (TOML or JSON, generated by the
   bot at publish time). Seed from a snapshot of the
   GitHub releases API; refresh on every successful publish.
2. Parse `Dependency::Version` through a real semver matcher
   (Cargo's `semver` crate covers caret / tilde / comparator
   sets / wildcards).
3. Resolver fetches the index once per build, picks the
   highest matching version for each entry, and feeds that
   exact version into the existing `release_url` path.

Not blocking the docs work — every package can keep pinning
exact versions today. Logged so the cascade-of-bumps cost
gets paid down once we're ready to invest in the resolver.
