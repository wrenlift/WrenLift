...
class MainWlift {
  ...
  async init() {
    ...
    // Shared canvas registry — `makeDomBridge` (creator) and the
    // shared `createGpuBridge` factory (consumer) read from the
    // same Map so a canvas created via @hatch:window is
    // referenceable by handle from @hatch:gpu's
    // `host_gpu_attach_canvas` without a separate cross-plugin
    // lookup. Each entry is `{ element, queue, closeRequested,
    // teardown, owned }`.
    const canvases = new Map(); // handle -> { element, queue, closeRequested, teardown }
    let nextCanvasHandle = 1;

    // ---------------------------------------------------------
    // GPU bridge — extracted into the shared `gpu-bridge.js`
    // module so worker.js can use the exact same imports object
    // against an `OffscreenCanvas` instead of a `HTMLCanvasElement`
    // (WebGPU's `getContext("webgpu")` works the same on both).
    // The factory carries the gpu device, preferred format,
    // resource registry, and lazy device-init internally; we
    // only hand it the host wasm exports + the canvas registry.
    const { bridge: gpuBridge, ensureGpuDevice } = createGpuBridge({ wasm, canvases });
    ...
    function makePluginImports(libName, getInstance) {
      ...
      // GPU bridge is the shared factory's already-built imports
      // object (extracted to gpu-bridge.js so worker.js can reuse
      // it). It doesn't need `getInstance` because none of its
      // calls touch plugin memory — slot reads go through host
      // memory via `wrenGetSlotString` etc.
      const gpu = gpuBridge;
      return {
        env: {
