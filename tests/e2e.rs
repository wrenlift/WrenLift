...
/// A call site that has not run when its function reaches the top tier
/// is dispatched over the classes defining the method, with no generic
/// call, and still records the class it meets for the next compile.
/// A second class defining the method keeps the compile from resolving
/// the site to one class up front.
#[test]
fn e2e_a_site_compiled_before_it_ran_dispatches_by_class_and_records_it() {
    use wren_lift::runtime::engine::{FuncId, TierState};
    if wren_lift::codegen::top_tier() == wren_lift::codegen::TopTier::Off {
        return;
    }
...
"#;
    ...
    let named = |vm: &VM| {
        ...
        table
            .iter()
            .filter(|ic| ic.snapshot().is_some_and(|s| s.names_function()))
            .count()
    };
    let before_named = named(&vm);
    let before_deopts = vm.engine.deopt_exits;
    vm.engine.collect_tier_stats = true;
    vm.engine.runtime_call_stats = Default::default();
    let then = "import \"main\" for K, T\nvar t = T.new()\nK.run(t, true)\nSystem.print(t.n)\n";
    assert!(matches!(
        vm.interpret("then", then),
        InterpretResult::Success
    ...
    assert_eq!(vm.take_output().trim(), "50");
    if vm.engine.deopt_exits != before_deopts {
        // The interpreter ran the site instead; it records its own kind.
        return;
    }
    assert_eq!(vm.engine.runtime_call_stats.wren_call_entries, 0);
    assert_eq!(named(&vm), before_named + 1);
}
